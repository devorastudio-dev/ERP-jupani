...
function normalizeProductPayload(values: ReturnType<typeof productSchema.parse>) {
  ...
  return {
    ...
    minimum_order_quantity:
      values.minimum_order_quantity && values.minimum_order_quantity > 0 ? values.minimum_order_quantity : null,
    minimum_order_unit: values.minimum_order_unit?.trim() || null,
    lead_time_hours: values.lead_time_hours && values.lead_time_hours > 0 ? values.lead_time_hours : null,
    order_guidelines: values.order_guidelines?.trim() || null,
    storefront_section_title: values.storefront_section_title?.trim() || null,
    storefront_badge_text: values.storefront_badge_text?.trim() || null,
...
export async function createProductAction(formData: FormData) {
  ...
  const parsed = productSchema.safeParse({
    ...
    minimum_order_unit: formData.get("minimum_order_unit"),
    lead_time_hours: formData.get("lead_time_hours"),
    accepts_flavor_selection: formData.get("accepts_flavor_selection") === "true",
    accepts_size_selection: formData.get("accepts_size_selection") === "true",
    accepts_theme_customization: formData.get("accepts_theme_customization") === "true",
    accepts_custom_message: formData.get("accepts_custom_message") === "true",
    accepts_event_date: formData.get("accepts_event_date") === "true",
    accepts_serving_count: formData.get("accepts_serving_count") === "true",
    accepts_variant_notes: formData.get("accepts_variant_notes") === "true",
    requires_manual_quote: formData.get("requires_manual_quote") === "true",
    order_guidelines: formData.get("order_guidelines"),
    storefront_section_title: formData.get("storefront_section_title"),
    storefront_badge_text: formData.get("storefront_badge_text"),
